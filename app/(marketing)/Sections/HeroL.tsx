...
function HeroL() {
    ...
    return (
        <section className='w-full max-sm:pb-6 pb-12 max-md:pb-12 border-b-2 border-neutral-100 text-center flex items- flex-col h-fit'>
            <motion.img initial={{ scale: 1 }} animate={{ scale: 1.1, translateY: [-15, 15] }} transition={{ translateY: { duration: 6, repeatType: 'mirror', repeat: Infinity }, scale: { duration: 1, repeat: 0 } }} alt='3d boy model' className='absolute w-48 max-sm:w-20 max-lg:w-32 max-md:w-24 top-28 max-sm:-left-8 max-md:left-2 left-8' src={Boy1.src} />
            <motion.img initial={{ scale: 1 }} animate={{ scale: 1.1, translateY: [-15, 15] }} transition={{ translateY: { duration: 6, repeatType: 'mirror', repeat: Infinity }, scale: { duration: 1, repeat: 0 } }} alt='3d boy model' className='absolute w-52 max-md:w-24 max-lg:w-40 top-28 max-sm:w-24 max-md:right-2 right-8  max-sm:-right-8' src={Boy2.src} />
